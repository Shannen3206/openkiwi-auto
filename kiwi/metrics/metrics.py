...
class Metric:
    ...
    def token_mask(self, batch, memory, t_type):
        target = self.get_target(batch, memory, t_type)     # tags [b, t] 真实值
        if self.PAD is not None:
            return target != self.PAD
        else:
            return torch.ones(target.shape, dtype=torch.uint8, device=target.device)

    def get_target(self, batch, memory, t_type):
        if t_type == 'valid':
            target = getattr(batch, self.target_name)   # tags [b, t] 真实值
        elif t_type == 'train':
            target = memory['tags']
    ...
    def get_token_indices(self, batch, memory, t_type):
        mask = self.token_mask(batch, memory, t_type)               # [b, t] tags为0或1 true；tags为2 false
        return mask.view(-1).nonzero().squeeze()    # 获取非0元素（True）的下标

    def get_predictions(self, model_out):
        predictions = model_out[self.target_name]
        return predictions

    def get_target_flat(self, batch, memory, t_type):
        target_flat = self.get_target(batch, memory, t_type).contiguous().view(-1)
        token_indices = self.get_token_indices(batch, memory, t_type)   # tags中非<pad>的下表集合
        return target_flat[token_indices]

    def get_predictions_flat(self, model_out, batch, memory, t_type):
        # predictions[b, t, 2]      predictions_flat[b*t, 2] 预测值
    ...
    def get_tokens(self, batch, memory, t_type):
        return self.token_mask(batch, memory, t_type).sum().item()
...
class CorrectMetric(Metric):
    ...
    def update(self, model_out, batch, memory, t_type, **kwargs):
        self.tokens += self.get_tokens(batch, memory, t_type)
        logits = self.get_predictions_flat(model_out, batch, memory, t_type)
        target = self.get_target_flat(batch, memory, t_type)
        _, pred = logits.max(-1)
        correct = target == pred
...
class F1Metric(Metric):
    ...
    def update(self, model_out, batch, memory, t_type, **kwargs):
        logits = self.get_predictions_flat(model_out, batch, memory, t_type)    # 预测值 [x, 2] 其中x表示tags为OK或BAD（不为pad）的数据个数
        target = self.get_target_flat(batch, memory, t_type)                    # 真实值 [x] x含义同上
        _, y_hat = logits.max(-1)   # max(-1)返回内层数组中的最大值，以及下表
        self.Y_HAT += y_hat.tolist()    # 预测值
        self.Y += target.tolist()       # 真实值
