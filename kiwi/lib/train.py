...
def train_from_options(options):
    ...
    with tracking_run:
        ...
        log(output_dir, config_options=vars(all_options), save_config=pipeline_options.save_config)

        # 读取数据 ###
        ### Ok是0，BAD是1
        times = all_options.pkl_read
        memory = []
        with open('data/h_state_gpu.pkl', 'rb') as fid:
            for i in range(times):
                a = pickle.load(fid)
                memory.append(a)
        for item in memory:
            alignments = process_align(item['align'])
            item['align'] = alignments
        # ###

        trainer = run(ModelClass, output_dir, pipeline_options, model_options, memory)
...
def run(ModelClass, output_dir, pipeline_options, model_options, memory):
    """
    ...
    # Data step
    fieldset = ModelClass.fieldset(wmt18_format=model_options.__dict__.get("wmt18_format"))
    ...
    # 使用opennmt-auto中得到的词表 ###
    other_vocabs = {}
    vocabs = torch.load('data/wmt17_vocab.pt')
    other_vocabs['source'] = vocabs[0][1]
    other_vocabs['target'] = vocabs[1][1]
    vocabs = utils.fields_to_vocabs(datasets[0].fields)
    other_vocabs['tags'] = vocabs['tags']
    # ###

    trainer = retrieve_trainer(ModelClass, pipeline_options, model_options, other_vocabs, output_dir, device_id)
    ...
    trainer.run(train_iter, valid_iter, memory, epochs=pipeline_options.epochs)

    return trainer


def retrieve_trainer(ModelClass, pipeline_options, model_options, vocabs, output_dir, device_id):
    """
    ...
    if isinstance(model, LinearWordQEClassifier):
        ...
        optimizer = OptimizerClass(model.parameters(), lr=pipeline_options.learning_rate)
        scheduler = None
...
def setup(output_dir, seed=42, gpu_id=None, debug=False, quiet=False):
    ...
    logging.info(
        "Inside experiment ID: {} ({})".format(tracking_logger.experiment_id, tracking_logger.experiment_name)
    )
    logging.info("Local output directory is: {}".format(output_dir))
    logging.info("Logging execution to MLflow at: {}".format(tracking_logger.get_tracking_uri()))
    ...
    logging.info("Artifacts location: {}".format(tracking_logger.get_artifact_uri()))
