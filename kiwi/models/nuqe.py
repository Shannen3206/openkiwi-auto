...
class NuQE(QUETCH):
    ...
    def forward(self, batch, memory, t_type=None):
        ...
        # target_input[b, tgt_t, win]   source_input[b, tgt_t, aligned, win]    nb_alignments[b, t]
        target_input, source_input, nb_alignments = self.make_input(batch, align_side, memory, t_type)
        ...
        h = torch.cat(feature_set, dim=-1)      # [b, t, 2*window*dim]
        h = self.dropout_in(h)                  # [b, t, 2*window*dim]
        h = F.relu(self.linear_1(h))            # [b, t, l1_dim]
        h = F.relu(self.linear_2(h))            # [b, t, l1_dim]
        h, _ = self.gru_1(h)                    # [b, t, l1_dim]
        h = F.relu(self.linear_3(h))            # [b, t, l2_dim]
        h = F.relu(self.linear_4(h))            # [b, t, l2_dim]
        h, _ = self.gru_2(h)                    # [b, t, l1_dim]
        h = F.relu(self.linear_5(h))            # [b, t, l3_dim]
        h = F.relu(self.linear_6(h))            # [b, t, l4_dim]
        h = self.dropout_out(h)                 # [b, t, l4_dim]
        h = self.linear_out(h)                  # [b, t, 2]
        outputs = OrderedDict()
