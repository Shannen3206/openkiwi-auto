...
class Model(nn.Module):
    ...
    def predict(self, batch, memory, t_type, class_name=const.BAD, unmask=True):
        model_out = self(batch, memory, t_type)
        predictions = {}
        ...
        for key in model_out:
            if key in [const.TARGET_TAGS, const.SOURCE_TAGS, const.GAP_TAGS]:
                ...
                class_probs = probs.index_select(-1, class_index.to(device=probs.device))
                ...
                # src_tuple = [src, src_len]
                # source = tuple(src_tuple)
                # batch.source = source
                #
                # tgt, tgt_len = batch.target
                # tgt = tgt.numpy()
                # tgt[tgt == 0] = 1
                # tgt = torch.from_numpy(tgt)
                # tgt_tuple = [tgt, tgt_len]
                # target = tuple(tgt_tuple)
                # batch.target = target
                ...
                if unmask:
                    ...
                    if key == const.GAP_TAGS:
                        ...
                        mask = torch.cat([mask.new_ones((mask.shape[0], 1)), mask], dim=1)
