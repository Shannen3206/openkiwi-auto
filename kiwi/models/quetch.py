...
class QUETCH(Model):
    ...
    def loss(self, model_out, target, memory, t_type):
        ...
        # (bs*ts, nb_classes)
        probs = model_out[output_name]                      # [b, t, 2] 预测值
        # (bs*ts, )
        if t_type == 'valid':
            y = getattr(target, output_name)                    # [b, t]    真实值
        elif t_type == 'train':
            y = memory['tags']

        predicted = probs.view(-1, self.config.nb_classes)  # [b*t, 2]
        y = y.view(-1)                                      # [b*t]
    ...
    def make_input(self, batch, side, memory, t_type):
        if t_type == 'valid' or t_type == None:
            target_input, target_lengths = getattr(batch, const.TARGET)     # target_input [b, src_t]
            source_input, source_lengths = getattr(batch, const.SOURCE)     # source_input [b, tgt_t]
            alignments = batch.alignments                                   # [b, 数量, 2]    list-list-tuple
        elif t_type == 'train':
            # change ###
            target_input = memory['tgt']
            source_input = memory['src']
            alignments = memory['align']
            # ###

        if self.config.predict_gaps and not self.config.predict_target:
            target_input = F.pad(target_input, pad=(0, 1), value=self.vocabs[const.TARGET].stoi[const.UNALIGNED])
            source_input = F.pad(source_input, pad=(0, 1), value=self.vocabs[const.SOURCE].stoi[const.UNALIGNED])

        # target_input[b, tgt_t, win]   source_input[b, src_t, win]
        target_input = convolve_tensor(target_input, self.config.window_size, self.config.target_padding_idx)
        source_input = convolve_tensor(source_input, self.config.window_size, self.config.source_padding_idx)

        if side == const.SOURCE_TAGS:
            alignments = [[alignment[::-1] for alignment in example_alignment] for example_alignment in alignments]
            target_input, nb_alignments = align_tensor(
                target_input, alignments,
                self.config.max_aligned, self.config.unaligned_idx, self.config.target_padding_idx,
                pad_size=source_input.shape[1],
            )
        else:
            # 经过下面这个函数后对齐了
            source_input, nb_alignments = align_tensor(
                source_input, alignments,
                self.config.max_aligned, self.config.unaligned_idx, self.config.source_padding_idx,
                pad_size=target_input.shape[1],
            )
        return target_input, source_input, nb_alignments

    def forward(self, batch, memory, t_type):
        ...
        target_input, source_input, nb_alignments = self.make_input(batch, align_side, memory)
