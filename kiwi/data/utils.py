...
def process_align(alignments):
    alignments = alignments.detach().cpu().numpy().tolist()
    new_align = []
    new_alignments = []
    for align in alignments:
        while align.__contains__(-1):
            align.remove(-1)
        new_align.append(align)
    for i in range(len(alignments)):
        a = torch.tensor(alignments[i], dtype=torch.int32)
        list_a = torch.split(a, 2)
        mm = [tuple(it.numpy().tolist()) for it in list_a]
        new_alignments.append(mm)
