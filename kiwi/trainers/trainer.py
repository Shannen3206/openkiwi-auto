...
class Trainer:
    def __init__(self, model, optimizer, checkpointer, log_interval=100, scheduler=None):
        """
        Args:
    ...
    def run(self, train_iterator, valid_iterator, memory, epochs=50):
        """
        Args:
            train_iterator:
        ...
        for epoch in range(self._epoch + 1, epochs + 1):
            ...
            self.train_epoch(train_iterator, valid_iterator, memory)
            self.stats.log()
            try:
                self.checkpointer(self, valid_iterator, memory, epoch=epoch, t_type='valid')    # 训练完成后的checkpointer传了epoch
            except EarlyStopException as e:
                logger.info(e)
                break
        self.checkpointer.check_out()

    def train_epoch(self, train_iterator, valid_iterator, memory):
        self.model.train()
        for i, batch in enumerate(tqdm(train_iterator, total=len(train_iterator), desc='Batches', unit=' batches', ncols=80)):
            self._step += 1

            outputs = self.train_step(batch, memory[i])    # ###
            self.stats.update(batch=batch, memory=memory[i], t_type='train', **outputs)
            self.stats.log(step=self._step)
            try:
                self.checkpointer(self, valid_iterator, memory[i], step=self._step, t_type='train')    # 训练一个epoch的过程中的checkpointer不传epoch
            except EarlyStopException as e:
                logger.info(e)
                break
        self._epoch += 1
    ...
    # memory是自编码器中隐层结果 ###
    def train_step(self, batch, memory):
        self.model.zero_grad()
        model_out = self.model(batch, memory, t_type='train')   # tags: [b, t, 2] ###
        loss_dict = self.model.loss(model_out, batch, memory, t_type='train')
        loss_dict[const.LOSS].backward()
        self.optimizer.step()
        return dict(loss=loss_dict, model_out=model_out)

    def eval_epoch(self, valid_iterator, memory, t_type, prefix='EVAL'):
        self.model.eval()
        self.stats.reset()
        with torch.no_grad():
            for batch in valid_iterator:
                outputs = self.eval_step(batch, memory, t_type)
                self.stats.update(batch=batch, memory=memory, t_type=t_type, **outputs)
        stats_summary = self.stats.wrap_up(prefix=prefix)
        self.model.train()
        return stats_summary

    def eval_step(self, batch, memory, t_type):
        model_out = self.model(batch, memory, t_type)
        loss_dict = self.model.loss(model_out, batch, memory, t_type)
        return dict(loss=loss_dict, model_out=model_out)

    def predict(self, valid_iterator, memory, t_type):
        self.model.eval()
        with torch.no_grad():
            predictions = defaultdict(list)
            for batch in valid_iterator:
                model_pred = self.model.predict(batch, memory, t_type)
