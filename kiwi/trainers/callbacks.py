...
class Checkpoint:
    ...
    def __call__(self, trainer, valid_iterator, memory, epoch=None, step=None, t_type=None):
        if self.must_eval(epoch=epoch, step=step):
            eval_stats_summary = trainer.eval_epoch(valid_iterator, memory, t_type)
            eval_stats_summary.log()
            if trainer.scheduler:
                trainer.scheduler.step(eval_stats_summary.main_metric_value())

            saved_path = self.check_in(trainer, eval_stats_summary, epoch=epoch, step=step)
            if saved_path:
                predictions = trainer.predict(valid_iterator, memory, t_type='valid')
                if predictions is not None:
                    save_predicted_probabilities(saved_path, predictions)
            elif self.early_stopping():
                raise EarlyStopException(
                    'Early stopping training after {} validations '
                    'without improvements on the validation set'.format(self.early_stop_patience)
                )
